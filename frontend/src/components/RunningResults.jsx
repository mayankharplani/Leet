import React from 'react'
import {
  CheckCircle2,
  XCircle,
  Clock,
} from "lucide-react";


const RunningResults = ({data}) => {

    const memoryArray = data.map((tc) => tc.memory);
    const avgMemory =
    memoryArray
      .map((m) => parseFloat(m)) // remove ' KB' using parseFloat
      .reduce((a, b) => a + b, 0) / memoryArray.length;
    let avgMemoryMb = avgMemory / 1024;
    
    const timeArray = data.map((tc) => tc.time);

    const avgTime = timeArray.map((t) => parseFloat(t)).reduce((a,b) => a+b,0)

    let status = true;
    const statusCheck = data.map((tc) => {
        if(!tc.passed){
            status = false;
        }
    })

   const passedTests = data.filter((tc) => tc.passed).length;
  const totalTests = data.length;
  const successRate = (passedTests / totalTests) * 100;

  return (
    <div className="space-y-6">
      {/* Overall Status */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-[var(--steel)] shadow-lg">
          <div className="card-body p-4">
            <h3 className="card-title text-sm">Status</h3>
            <div
              className={`text-lg font-bold ${
                status  ? "text-success" : "text-error"
              }`}
            >
              {status ? "Accepted" : "Wrong Answer"} 
            </div>
          </div>
        </div>

        <div className="bg-[var(--steel)] shadow-lg">
          <div className="card-body p-4">
            <h3 className="card-title text-sm">Success Rate</h3>
            <div className="text-lg font-bold">{successRate.toFixed(1)}%</div>
          </div>
        </div>

        <div className="bg-[var(--steel)] shadow-lg">
          <div className="card-body p-4">
            <h3 className="card-title text-sm flex items-center gap-2">
              <Clock className="w-4 h-4" />
              Avg. Runtime
            </h3>
            <div className="text-lg font-bold">{avgTime.toFixed(3)} s</div>
          </div>
        </div>

        <div className="bg-[var(--steel)] shadow-lg">
          <div className="card-body p-4">
            <h3 className="card-title text-sm flex items-center gap-2">
              Avg. Memory
            </h3>
            <div className="text-lg font-bold">{avgMemoryMb.toFixed(1)} MB</div>
          </div>
        </div>
      </div>

      {/* Test Cases Results */}
      <div className="bg-[var(--steel)] shadow-xl">
        <div className="card-body">
          <h2 className="card-title mb-4" style={{color: "var(--cream)"}}>Test Cases Results</h2>
          <div className="overflow-x-auto">
            <table className="table table-zebra w-full">
              <thead>
                <tr>
                  <th>Status</th>
                  <th>Expected Output</th>
                  <th>Your Output</th>
                  <th>Memory</th>
                  <th>Time</th>
                </tr>
              </thead>
              <tbody>
                {data.map((testCase) => (
                  <tr key={testCase.id}>
                    <td>
                      {testCase.passed ? (
                        <div className="flex items-center gap-2 text-success">
                          <CheckCircle2 className="w-5 h-5" />
                          Passed
                        </div>
                      ) : (
                        <div className="flex items-center gap-2 text-error">
                          <XCircle className="w-5 h-5" />
                          Failed
                        </div>
                      )}
                    </td>
                    <td className="font-mono">{testCase.expected}</td>
                    <td className="font-mono">{testCase.stdout || "null"}</td>
                    <td>{(parseFloat(testCase.memory)/ 1024).toFixed(2)} MB</td>
                    <td>{parseFloat(testCase.time).toFixed(2)} Sec</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
  );
}

export default RunningResults